...
output_buffer = io.StringIO("No log available currently.")
sys.stdout = output_buffer  # Redirect print statements to the buffer
...
# === Write the UI Code below ===
st.title("Financial Report Generation")
st.write("This application generates a financial report based on the stock prices and news headlines of the stocks provided.")

stocks = st.text_input("Enter stock tickers (comma separated) you want to analyze as per Yahoo Finance site:\n"
"\ne.g. AAPL for Apple Inc., HDFCBANK.NS for HDFC Bank, etc.")
start_button = st.button('Generate Report')

# Initialize session state for markdown_bytes
if "markdown_report" not in st.session_state:
    st.session_state.markdown_report = None
    st.session_state.trend_img = None
    st.session_state.log_output = None

# Initiate the analysis if the start button is clicked
if start_button is True:
    from datetime import datetime
    date_str = datetime.now().strftime("%Y-%m-%d")

    # Reset markdown_report and trend_img to None
    st.session_state.markdown_report = None
    st.session_state.trend_img = None
    st.session_state.log_output = None
    ...
    financial_tasks = [
        ...
        * Debt/Eq
        * ROE
        * Analyze the correlation between the stocks if there are more than one stock
        Do not use a solution that requires an API key. Retrieve most data starting with Yfinance library.
        Use 'Close' price instead of 'Adj Close' price in the downloaded data.
        ...
        Retrieve news headlines using python and return them. Use the full name stocks to retrieve headlines. 
        Retrieve at least 10 headlines per stock. Do not use a solution that requires an API key. Do not perform a sentiment analysis.""",
    ]
            ...
            [
                {
                    ...
                    "clear_history": False,
                    "max_turns": 3,
                    "carryover": "Wait for confirmation of code execution before terminating the conversation."
                        "If no change in the code file, don't regenerate the code. Instead instruct to re-run the previous code file."
                        "Verify that no column in retrieved data is completely composed of NaN values. Use dataframe.isnull().all().any() to check."
                        "Reply TERMINATE with no succeeding characters in the end when everything is done."
                },
                {
                    "sender": user_proxy_auto,
...
if st.session_state.markdown_report:
    # Diaplay the trend image & Markdown report
    st.image(st.session_state.trend_img)
    st.markdown(st.session_state.markdown_report)

    # Convert the Markdown content to bytes
    markdown_bytes = st.session_state.markdown_report.encode("utf-8")

    # Display the download button for only report download
    # st.download_button(
    ...
    # Display the download button for both report & image
    with zipfile.ZipFile("Financial_Report.zip", "w") as zip_file:
        zip_file.writestr("Financial_Report.md", markdown_bytes)
        # Add the image to zipfile without the path information
        zip_file.write(st.session_state.trend_img, "Normalized_Prices.png")
       
    st.download_button(
        label="📥 Download Report",
        data=open("Financial_Report.zip", "rb"),
        file_name="Financial_Report.zip",
...
# Capture the logs if st.session_state.log_output is None
if st.session_state.log_output is None:
    st.session_state.log_output = output_buffer.getvalue()
